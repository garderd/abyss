	...
	BX.Sale.OrderAjaxComponent = {
		...
		getBlockFooter: function(node)
		{
				...
				buttons.push(
					BX.create('A', {
						props: {
							...
							className: 'pull-left uk-button-default busket_button_var_2'
						},
			...
			if (!isLastNode)
			{
				buttons.push(
					BX.create('A', {
						props: {href: 'javascript:void(0)', className: 'pull-left uk-button uk-button-default busket_button'},
		...
		createBasketItemContent: function(data)
		{
			...
			return BX.create(
				...
				children: [
					...
					BX.create('DIV', {props: {className: 'bx-soa-item-count'}, html: 'x' + data.QUANTITY}),
					BX.create('DIV', {props: {className: 'bx-soa-item-price-summ'}, html: data.SUM}),
				]
			});
		}, 
		...
		createBasketItemColumn: function(column, allData, active)
		{
			...
			return BX.create(
				'DIV', {
					props: {
						className: 'bx-soa-item-td  bx-soa-item-td--c5'
					},
					children: [
						BX.create(
							'DIV',
		...
		createPaySystemItem: function(item)
		{
			...
			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company col-lg-12'},
		...
		editTotalBlock: function()
		{
			...
			if(parseInt(total.BASKET_POSITIONS) !== 0 && total.BASKET_POSITIONS != ""){
				var count_tovars = parseInt(total.BASKET_POSITIONS);
				count_tovars += " шт.";
			}
			else{
				 count_tovars = "";
			...
			{
				if(parseInt(total.ORDER_WEIGHT) !== 0 && total.ORDER_WEIGHT_FORMATED != ""){

					var weight_tovars = ", ";  
					weight_tovars += total.ORDER_WEIGHT_FORMATED;
				}
				else{
					weight_tovars = "";
				}
			...
			this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_SUMMARY') + " " + count_tovars + weight_tovars , priceHtml, params));
			...
			if (!this.result.SHOW_AUTH)
			{
				this.totalInfoBlockNode.appendChild(
					BX.create('DIV', {
						props: {className: 'bx-soa-cart-total-button-container' + (!showOrderButton ? ' hidden-xs' : '')},
						children: [
							BX.create('A', {
								props: {
									href: 'javascript:void(0)',
